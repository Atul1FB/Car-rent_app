...
const Hero = () => {
  ...
  return (
    ...
    className="h-screen flex flex-col items-center justify-center gap-14 bg-light text-center px-4">
      ...
      animate={{y:0,opacity:1}}
      transition={{duration:0.8,delay:0.2}}
       className="text-4xl md:text-5xl font-semibold  text-yellow-700 font-heading">
        Luxury Cars on Rent
      </motion.h1>
      ...
      onSubmit={handleSearch}
        className="flex flex-col gap-6 md:flex-row md:items-end justify-center 
                   p-6 rounded-lg md:rounded-full w-full max-w-[90%] md:max-w-4xl bg-white 
                   shadow-[0px_8px_20px_rgba(0,0,0,0.1)]"
      >
        {/* Grouping Inputs + Button Together */}
        <div className="flex flex-col md:flex-row items-start md:items-end gap-6 md:gap-8 w-full">
          ...
          <div className="flex flex-col items-start gap-2 w-full md:w-auto">
            ...
            <input value={pickupDate} onChange={e=>setPickupDate(e.target.value)}
              ...
              className="text-sm text-gray-700 border border-gray-300 rounded-md px-3 py-2 w-full md:w-44"
              required
            />
          </div>
